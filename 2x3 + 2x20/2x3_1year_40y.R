  ...
  #Load Y's Market return 1975-2015
  library(sandwich)
  library(lmtest)
  library(xtable)
  
  setwd("C:/Users/jesper/OneDrive/Master Thesis/Data testing/rstuff/2x3 + 2x20")
  ...
  #Load X's 2x3 size/BM 1975-2015
  Firms <- read.table("6_Portfolios_2x3.txt",
                      skip=2947,
                      nrows=498,
                      na.strings=c(-99.99,99.99,0),
                      header=FALSE)
  
  Avgme<- read.table("6_Portfolios_2x3.txt",
                        skip=4028,
                        nrows = 498,
                        na.strings=c(-99.99,99.99,0),
                        header=FALSE)
  
  Sumbe <- read.table("6_Portfolios_2x3.txt",
                  skip=4772,
                  nrows = 41,
                  na.strings=c(-99.99,99.99,0),
                  header=FALSE)
  ...
  bm <- matrix(1,nrow=((nrow(Sumbe)-1)*12),ncol=ncol(Summe))
  ...
  xrets <- read.table("Yrets.txt")
  
  x <- cbind(log(bm),log(xrets[1:(nrow(xrets)-12),2:ncol(xrets)]*0.01+1))
  ...
  #newey west
  fit4 <- lm(y[2:(length(y)-11)]~f1[1:(length(y)-12)])
  ...
  plot(y=oosr2,x=seq(fin-(length(oosr2)*1/12)+1/12,to=fin,by=1/12),type="l",
       ylab="Out-of-sample R.squared", xlab="Time",main="2x3 Pfs",xaxt="n",ylim=c(-30,30))
  abline(0,0)
  axis(1,xaxp=c(1940,2010,7),las=2)
  pencnew <- pt(-abs(encnew),(length(oosr2)-1))*2
  write.table(cbind(frame,c(oosr2,rep(NA,60)),c(pencnew,rep(NA,60))),"frame2x3IS_1y_40y.txt",
