  ...
  setwd("C:/Users/jesper/OneDrive/Master Thesis/Data testing/rstuff/10x10 + 2x20")
  ...
  #x2 <- read.table("100bm.txt",sep=",")
  #y1 <- read.table("y1y.txt",sep=";")
  #y1<- as.numeric(y1[,])
  #standardize x
  ...
  plot(y=oosr2,x=seq(fin-(length(oosr2)*1/12)+1/12,to=fin,by=1/12),type="l",
       ylab="Out-of-sample R.squared", xlab="Time",main="10x10 Pfs",xaxt="n",ylim=c(-30,30))
  abline(0,0)
  axis(1,xaxp=c(1940,2010,7),las=2)
  pencnew <- pt(-abs(encnew),(length(oosr2)-1))*2
  write.table(cbind(frame,c(oosr2,rep(NA,60)),c(pencnew,rep(NA,60))),"frame1010IS_1y_40y.txt",
