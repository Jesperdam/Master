...
write.table(cbind(Factmont[61:(nrow(na.omit(framed))+60),1],na.omit(framed)),"oosy2005_1m_indu.txt",
            row.names=F)
...
for (l in 1:ncol(y)){
  ...
  framed[1:length(oosr2),(l*2-1):(l*2)] <-cbind(oosr2,pt(-abs(encnew),200)*2)
write.table(cbind(Factmont[73:(nrow(na.omit(framed))+72),1],na.omit(framed)),"oosy2005_1y_indu.txt",
            row.names=F)
