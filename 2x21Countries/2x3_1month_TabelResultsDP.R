...
Factmont<- read.table("Portfolios_Formed_on_D-P.txt",
                      skip=590,
                      nrows=492,
...
write.table(cbind(Factmont[61:(nrow(na.omit(framed))+60),1],na.omit(framed)),"oosy2005_1m_dp.txt",
            row.names=F)
...
write.table(cbind(Factmont[73:(nrow(na.omit(framed))+72),1],na.omit(framed)),"oosy2005_1y_dp.txt",row.names=F)
