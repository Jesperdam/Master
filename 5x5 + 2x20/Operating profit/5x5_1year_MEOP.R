  ...
  #Load X's 10x10 size/BM 1926-2016
  Firms <- read.table("25_Portfolios_BEME_OP_5x5.txt",
                      skip=1407,
                      nrows=632,
                      na.strings=c(-99.99,99.99,0),
                      header=FALSE)
  
  Avgme<- read.table("25_Portfolios_BEME_OP_5x5.txt",
                        skip=2044,
                        nrows = 632,
                        na.strings=c(-99.99,99.99,0),
                        header=FALSE)
  
  Sumbe <- read.table("25_Portfolios_BEME_OP_5x5.txt",
                  skip=2797,
                  nrows = 53,
                  na.strings=c(-99.99,99.99,0),
                  header=FALSE)
  ...
  plot(y=oosr2,x=seq(fin-(length(oosr2)*1/12)+1/12,to=fin,by=1/12),type="l",
       ylab="Out-of-sample R.squared", xlab="Time",main="5x5 Pfs")
abline(0,0)
pencnew <- (1-pt(encnew,(length(oosr2)-1)))*2
  write.table(cbind(frame,c(oosr2,rep(NA,60))),"frame5x5MEOP_1y.txt",
              row.names=F)
