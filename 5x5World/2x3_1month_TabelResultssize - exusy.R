...
for (l in 1:ncol(y)){
  ...
  oosr2<- rep(NA,(nrow(frame)-24))
  encnew <- rep(NA,(nrow(frame)-24))
  for (i in 1:(nrow(frame)-24))
  {
...
write.table(cbind(Factmont[73:(nrow(na.omit(framed))+72),1],na.omit(framed)),"oosy2005_y_bm_exus.txt",
            row.names=F)
