...
Factmont<- read.table("Portfolios_Formed_on_INV.txt",
                      skip=342,
                      nrows=308,
...
write.table(cbind(Factmont[61:(nrow(na.omit(framed))+60),1],na.omit(framed)),"oosy2005_m_inv_ex.txt",
            row.names=F)
xtable(rbind(cbind(Factmont[72:(nrow(na.omit(framed))+71),1],na.omit(framed))[104,2:3],
             ...
             cbind(Factmont[72:(nrow(na.omit(framed))+71),1],na.omit(framed))[104,10:11]),digits=3)
