...
for (l in 1:ncol(y)){
  ...
  for (i in 60:(nrow(y)-1)) {
    ...
    if (sum(!is.na(tempr))>60) {
      ...
      #final stage 1m
      coefs <- coef(lm(tempr[2:(nrow(xoos))]~f1[1:(nrow(xoos)-1)]+f2[1:(nrow(xoos2)-1)]))
      
      #prediktor
      prf[i+1] <- coefs[1]+coefs[2]*f1[i]+coefs[3]*f2[i]
...
write.table(cbind(oosres[1:nrow(na.omit(oos)),1:3],na.omit(oos)[,]),"scand_oos.txt",row.names=F)
