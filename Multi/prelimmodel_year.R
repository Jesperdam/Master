...
#finding phis
fac <- matrix(1,ncol=15,nrow=(nrow(x)-12))
for (l in 1:5){
  tempy<- y[,6+l]
  for (k in 1:3){
    if (k==1){
...
resmatrix <- matrix(0,ncol=35,nrow=(nrow(x)-13))
rsq <- rep(1,35)
...
for (l in 1:5)
{
  for(k in 1:3)
  {
    model <- lm(y[2:nrow(fac),l+6]~fac[1:(nrow(fac)-1),l*3-3+k])
    beta1f[,l*3-3+k] <- coef(model)[2]
    resmatrix[,l*3-3+k] <- residuals(model)
    rsq[l*3-3+k] <- summary(model)$r.squared
    nyw <- coeftest(model, vcov.=NeweyWest(model, prewhite=F,lag=11))
    pnw[1,l*3-3+k] <- pt(-abs(nyw[2:dim(nyw)[1],3]),(nrow(y)-1))*2
  }
}
for (l in 1:5)
{
  model <- lm(y[2:nrow(fac),l+6]~fac[1:(nrow(fac)-1),l*3-2]+fac[1:(nrow(fac)-1),l*3-1])
  ...
  model <- lm(y[2:nrow(fac),l+6]~fac[1:(nrow(fac)-1),l*3-2]+fac[1:(nrow(fac)-1),l*3])
  ...
  model <- lm(y[2:nrow(fac),l+6]~fac[1:(nrow(fac)-1),l*3]+fac[1:(nrow(fac)-1),l*3-1])
  ...
  model <- lm(y[2:nrow(fac),l+6]~fac[1:(nrow(fac)-1),l*3-2]+fac[1:(nrow(fac)-1),l*3-1]+fac[1:(nrow(fac)-1),l*3])
...
relfac <- cbind(fac[,3],rep(0,nrow(fac)),fac[,5],fac[,6],fac[,8],rep(0,nrow(fac)),fac[,11],rep(0,nrow(fac)),fac[,15],rep(0,nrow(fac)))
tx1 <- relfac[1:(nrow(relfac)-1),7]
tx2 <- relfac[1:(nrow(relfac)-1),8]
ty <- y[2:(nrow(fac)),10]
fit1 <- lm(ty~tx1+x$caymon[1:(nrow(x)-13)])#lettauludvigson
nyw1 <- coeftest(fit1, vcov.=NeweyWest(fit1, prewhite=F,lag=11))[,2]
fit2 <- lm(ty~tx1+x$infl[1:(nrow(x)-13)])#goyalwelch
nyw2 <- coeftest(fit2, vcov.=NeweyWest(fit2, prewhite=F,lag=11))[,2]
fit3 <- lm(ty~tx1+y[1:(nrow(x)-13),4])#rapach
nyw3 <- coeftest(fit3, vcov.=NeweyWest(fit3, prewhite=F,lag=11))[,2]
fit4 <- lm(ty~tx1+y[1:(nrow(x)-13),6])#rapach
nyw4 <- coeftest(fit4, vcov.=NeweyWest(fit4, prewhite=F,lag=11))[,2]
fit5 <- lm(ty~tx1+x$corpr[1:(nrow(x)-13)])    
nyw5 <- coeftest(fit5, vcov.=NeweyWest(fit5, prewhite=F,lag=11))[,2]
fit6 <- lm(ty~tx1+x$caymon[1:(nrow(x)-13)]+x$infl[1:(nrow(x)-13)]+
             y[1:(nrow(x)-13),6]+y[1:(nrow(x)-13),4]+x$corpr[1:(nrow(x)-13)]+x$corpr[1:(nrow(x)-13)])

nyw6 <- coeftest(fit6, vcov.=NeweyWest(fit6, prewhite=F,lag=11))[,2]
...
stargazer(fit1,fit2,fit3,fit4,fit5,fit6,se=list(nyw1,nyw2,nyw3,nyw4,nyw5,nyw6),dep.var.labels = "Return on the World portfolio",
          covariate.labels = c("2x20 Factor","cay","US inflation","Return on US","Return on Europe","Corp rate US","Constant"),
